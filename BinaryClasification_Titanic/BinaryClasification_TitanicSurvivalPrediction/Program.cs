...
namespace BinaryClasification_TitanicSurvivalPrediction
{
    ...
    internal static class Program
    {
        ...
        private static async Task Main(string[] args)
        {
            ...
            var model = await TrainAsyncBitcoin();

            // STEP2: Test accuracy
            EvaluateBitcoin(model);

            // STEP 3: Make a prediction
            var prediction = model.Predict(TestBitcoinDecisionData.Trade);

            Console.WriteLine($"I have to trade? {(TestBitcoinDecisionData.Trade.Type == 1 ? "Sell" : "Buy")}   Predicted: {(prediction.Decision)}");

            Console.ReadLine();
        }

        public static async Task<PredictionModel<BitCoinData, BitcoinPrediction>> TrainAsyncBitcoin()
        {
        ...
        private static void EvaluateBitcoin(PredictionModel<BitCoinData, BitcoinPrediction> model)
        {
